    ...
    <div class="card-wrap">
        ...
        <div class="card border-0 shadow card--register" id="register">
            <div class="card-body">
                ...
                <p>或者使用您的电子邮箱进行注册</p>
                <form action=<%=path+"/admin/reg.action"%> method="post" id="regform">
                    <div class="form-group">
                        <input class="form-control" type="text" placeholder="名称" required="required" name="userName"/>
                    </div>
                    <div class="form-group">
                        <input class="form-control" type="email" placeholder="邮箱" required="required" name="userEmall"/>
                    </div>
                    <div class="form-group">
...
<%--                        <s:select list="#request.map"  label="abc" listKey="key" listValue="value" value="#request.aa"  headerKey="0" headerValue="aabb"></s:select>--%>
                    ...
                    <div class="form-group">
                        <input class="form-control" type="password" placeholder="密码" required="required" name="userPwd"/>
                    </div>
                    <div class="form-group">
                        <div class="field">
    ...
    $(function () {
        ...
        $("#filess").on(
            "click",function () {
                ...
                $.ajaxFileUpload({
                    url: getRootPath()+"/admin/upload.action", //用于文件上传的服务器端请求地址
...
//		jQAJAX("../ImgCodeServlet", sendData, function (msg) {
                $("#imgCode").attr("src",getRootPath()+"/image/imageyzm.action?time=" + new Date().getTime());
//		})
            }
        });
        //点击注册时需要给隐藏域赋值
        ...
        $("#frontreg").on(
            "click",function () {
                ...
                jQAJAXS(getRootPath()+"/admin/regDropdownBox.action", send, function (msg) {
                        // var ob = JSON.parse(msg);
                        // var ob = eval('(' + msg + ')');
                        var listedu = msg.education;
                        var listocc = msg.occupation;
                        // alert(listedu);
        ...
        $("#loginusername").on(
            "blur", function () {
                ...
                if (username == '') {
                    ...
                    // alert(username);
                    jQAJAXS(getRootPath()+"/admin/userTest.action", send, function (msg) {
                        // var ob = eval('(' + msg + ')');
                        if (msg.sss!="可以登录")
                        {
                            $("#loginusername").val("");
        ...
        $("#frontlogin").on(
            "click", function () {
                ...
                jQAJAXS(getRootPath()+"/admin/userexecute.action", send, function (msg) {
                    // var ob = eval('(' + msg + ')');
                    if((msg.ver)=="登录成功"){
                        // alert(ob.ver);http://localhost:8080/firstweb/jsp/home.jsp"
                        window.location.href = getRootPath()+"/admin/userMenu.action";
                    }else{
                        $("#imgCode").attr("src",getRootPath()+"/image/imageyzm.action?time=" + new Date().getTime());
                        alert(msg.ver);
                    }
